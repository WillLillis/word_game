...
pub fn build(b: *std.Build) !void {
    ...
    if (target.query.os_tag == .emscripten) {
        const exe_lib = rlz.emcc.compileForEmscripten(b, "word_game", "src/main.zig", target, optimize);
        ...
        const run_option = b.step("run", "Run word_game");
        run_option.dependOn(&run_step.step);
        return;
    }

    const exe = b.addExecutable(.{ .name = "word_game", .root_source_file = .{ .path = "src/main.zig" }, .optimize = optimize, .target = target });
